...
def transfer_pretrain_han(fine_tuning_model: HGTSynergy, dim_drug_features, dim_drug_embs, dim_hidden, dim_ddi_encode,
                          dim_ddi_class, p_dropout, heads, pretrain_pth):
    print('开始加载HAN预训练模型...')
...
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, epochs, model_pth, name, model_type,
                ddi_graph=None):
    logger = logging.getLogger(name)
    logger.info(f'开始{name}训练...')
    min_train_avg_loss = 1e9
    min_val_avg_loss = 1e9
    writer = SummaryWriter(comment=f'{name}训练')
    for epoch in range(1, epochs + 1):
        ...
        for line in pbar_train_loader:
            ...
            outputs = model(drugA_features_tensor, drugB_features_tensor, cell_features_tensor, ddi_graph,
                            ddi_index_tensor)
        ...
        writer.add_scalars(f'{name}训练过程Loss', {'train_loss': epoch_train_avg_loss, 'val_loss': epoch_val_avg_loss},
                           epoch)

        tqdm.write(
            '第{}轮{}训练，Train_Loss: {}, Val_Loss: {}'.format(epoch, name, epoch_train_avg_loss, epoch_val_avg_loss))
