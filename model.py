...
class Predictor(nn.Module):
    def __init__(self, dim_drug_embs, dim_cell_embs, dim_ddi_encode, p_dropout, hidden_size):
        ...
        self.drug_atten_layer = nn.MultiheadAttention(dim_ddi_encode, num_heads=4, dropout=p_dropout, batch_first=True)
    ...
    def forward(self, drugA_embs, drugB_embs, cell_embs, ddi_embs):
        ...
        ddi, drugA, drugB = process_ddi(drugA_ddi_embs.unsqueeze(1), drugB_ddi_embs.unsqueeze(1), ddi_embs.unsqueeze(1))
        drugA = self.drug_atten_layer(query=drugA, key=ddi, value=ddi)[0]
        drugA = F.layer_norm(drugA, normalized_shape=(drugA.size(2),))
        drugA_ddi_embs = drugA.squeeze(1)
        drugB = self.drug_atten_layer(query=drugB, key=ddi, value=ddi)[0]
        drugB = F.layer_norm(drugB, normalized_shape=(drugB.size(2),))
        drugB_ddi_embs = drugB.squeeze(1)
...
class HANModel(nn.Module):
    ...
    def forward(self, data, pretrain_start_index, pretrain_end_index, edge_list=None):
        ...
        x_ddi = torch.cat([x_ddi, x_ddi], dim=1)
