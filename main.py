...
def nest_cv(lr_list, dropout_list, hidden_size_list, model_pth,
            pretrain_pth=None, no=None):
    ...
    for test_fold in range(5):
        ...
        for LR, DROPOUT, HIDDEN_SIZE in itertools.product(lr_list, dropout_list, hidden_size_list):
            ...
            for val_fold in range(5):
                ...
                logging.info("================================================================")
                logging.info(
                    f"嵌套交叉验证, test_fold={test_fold}, lr={LR,}, dropout={DROPOUT}, hidden_size={HIDDEN_SIZE}, val_fold={val_fold}")
        ...
        test_mse, test_rmse, test_pcc = test_model(model, outer_test_loader, criterion, optimizer,
                                                   name=f'no={no}_testFold={test_fold}',
        ...
        logging.info("*******************************************************")
        logging.info(
            f'test_fold={test_fold}的嵌套训练完成。参数组合: best_lr={best_lr}, best_dropout={best_dropout}, best_hidden_size={best_hidden_size}。test_mse为{test_mse}, ' +
    ...
    logging.info("******************************************************")
    logging.info(
        f"嵌套交叉验证全部完成。mse={mse_mean}±{mse_std}, rmse={rmse_mean}±{rmse_std}, pcc={pcc_mean}±{pcc_std}")
...
if __name__ == '__main__':
    ...
    nest_cv(lr_list=[0.45e-3], dropout_list=[0.20], hidden_size_list=[2048, 4096, 8192],
        model_pth='./checkpoints/HGTSynergy.pth',
        pretrain_pth='./checkpoints/pretrained_han.pth',
        no='1')
